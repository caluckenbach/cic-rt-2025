...
export async function requestBenefitAPI(userId: string, benefitId: string, vehiclePrice?: number) {
  try {
    ...
    let calculatedCosts = {};
    if (benefit.simulationConfig?.hasCalculator && benefit.simulationConfig?.formula && vehiclePrice) {
      calculatedCosts = calculateMobilityCosts(vehiclePrice, benefit.simulationConfig.formula);
...
export async function simulateBenefitAPI(benefitId: string, vehiclePrice: number) {
  try {
    const benefit = getBenefitFromCatalog(benefitId);
    if (!benefit) {
      throw new Error('Benefit not found');
    ...
    if (!benefit.simulationConfig?.hasCalculator || !benefit.simulationConfig?.formula) {
      throw new Error('Benefit does not support simulation');
    }

    return calculateMobilityCosts(vehiclePrice, benefit.simulationConfig.formula);
  } catch (error) {
