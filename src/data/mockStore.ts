...
function seedDemoUserBenefits() {
	...
	for (const benefit of benefitCatalog) {
		...
		const netCost =
			grossCost !== undefined
				? Number((grossCost * 0.6).toFixed(2))
				: undefined;
...
export function getAllUserBenefits(userId: string) {
	return Array.from(mockStore.userBenefits.values()).filter(
		(ub) => ub.userId === userId,
	);
}

// Create or update user benefit
export function upsertUserBenefit(
	userId: string,
	benefitId: string,
	data: Partial<UserBenefit>,
) {
	const key = `${userId}:${benefitId}`;
	...
	const userBenefit: UserBenefit = {
		...
		vehiclePrice:
			data.vehiclePrice !== undefined
				? data.vehiclePrice
				: existing?.vehiclePrice,
		grossCost:
			data.grossCost !== undefined ? data.grossCost : existing?.grossCost,
...
export function getActiveBenefitsForUser(userId: string) {
	return Array.from(mockStore.userBenefits.values()).filter(
		(ub) => ub.userId === userId && ub.status === "ACTIVE",
	);
}
...
export function calculateTotalSavings(userId: string) {
	...
	return activeBenefits.reduce(
		(total, benefit) => total + (benefit.savings || 0),
		0,
	);
}
