...
export function BenefitsMarketplace() {
  ...
  const [activeSummary, setActiveSummary] = useState<ActiveBenefitsSummary | null>(null);
  ...
  const fetchBenefitsData = useCallback(async () => {
    ...
    try {
      ...
      setError(
        error instanceof Error ? error.message : "Unable to load benefits right now."
      );
    } finally {
      setLoading(false);
    }
  ...
  const simulateBenefit = async (benefitId: string) => {
    ...
    try {
      ...
      setError(
        error instanceof Error ? error.message : "Unable to simulate this benefit."
      );
    } finally {
      setSimulating(null);
    }
  ...
  const requestBenefit = async (benefitId: string) => {
    ...
    try {
      ...
      // Update local state
      setBenefits(prev =>
        prev.map(b => (b.id === benefitId ? { ...b, status: "REQUESTED" } : b))
      );
  ...
  const activateBenefit = async (benefitId: string) => {
    try {
      ...
      // Update local state
      setBenefits(prev =>
        prev.map(b => (b.id === benefitId ? { ...b, ...result, status: "ACTIVE" } : b))
      );

      setActiveSummary(prev => {
        const current = prev?.activeBenefits ?? [];
        const existingIndex = current.findIndex(b => b.benefitId === result.benefitId);
        const nextActiveBenefits =
          existingIndex >= 0
            ? current.map(b => (b.benefitId === result.benefitId ? result : b))
            : [...current, result];
  ...
  const categorizedBenefits = useMemo(() => {
    const available = benefits.filter(b => !b.status || b.status === "AVAILABLE");
      ...
      <CardContent>
        ...
        <Tabs defaultValue="available" className="w-full">
          <TabsList className="grid w-full grid-cols-3">
            ...
            <TabsTrigger value="requested">Pending ({requested.length})</TabsTrigger>
                    ...
                    {benefit.simulationConfig?.hasCalculator && (
                      <div className="space-y-3 mb-4 p-3 rounded-lg bg-muted">
                        ...
                        <Input
                          ...
                          onChange={(e) => handleVehiclePriceChange(benefit.id, e.target.value)}
                          className="mb-2"
                        />
                        <div className="flex gap-2">
                          <Button 
                            ...
                            disabled={!vehiclePrices[benefit.id] || simulating === benefit.id}
                          >
                            {simulating === benefit.id ? 'Calculating...' : 'Calculate'}
                          </Button>
                          <Button
                            size="sm"
                          ...
                          >
                            {requesting === benefit.id ? "Requesting..." : "Request Benefit"}
                          </Button>
                        </div>
                        {simulationResults[benefit.id] && (
                          <div className="mt-3 p-3 rounded-md bg-background text-sm">
                            <div className="flex justify-between mb-1">
                              ...
                              <span className="font-medium">€{simulationResults[benefit.id].grossCost.toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between mb-1">
                              <span className="text-muted-foreground">Your Cost:</span>
                              <span className="font-medium">€{simulationResults[benefit.id].netCost.toFixed(2)}</span>
                            </div>
                            <div className="flex justify-between font-bold text-green-600">
                              <span>Savings:</span>
                              <span>€{simulationResults[benefit.id].savings.toFixed(2)}</span>
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    {benefit.fixedValue && (
                      <div className="space-y-3 mb-4 p-3 rounded-lg bg-muted text-center">
                        ...
                        <div className="text-xs text-green-600 font-medium">
                          Save €{(benefit.fixedValue * 0.4).toFixed(2)} with 40% tax savings
                        </div>
                        <Button
                          size="sm"
                          className="w-full"
                  ...
                  <CardContent>
                    ...
                    <div className="mt-4 space-y-2 text-sm">
                      ...
                      <Button
                        ...
                        disabled={requesting === benefit.id}
                      >
                        {requesting === benefit.id ? "Activating..." : "Activate Benefit"}
                      </Button>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="md:col-span-2 lg:col-span-3 bg-gradient-to-r from-green-50 to-blue-50">
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="text-lg font-semibold text-green-800">Total Monthly Savings</h3>
