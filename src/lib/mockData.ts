...
const normalizeCatalogEntry = (
	...
	if (
		entry.hasCalculator &&
		entry.simulationFormula &&
		!entry.simulationConfig
	) {
		return {
			...entry,
			simulationConfig: {
...
export const benefitCatalog: BenefitCatalogEntry[] = (
	benefitsCatalogJson as BenefitCatalogEntry[]
).map(normalizeCatalogEntry);

export const mockUsers: MockUser[] = demoUserProfileJson as MockUser[];

export const mockBenefits: MockBenefit[] =
	demoUserBenefitsJson as MockBenefit[];
...
export function getBenefitsByUserId(userId: string): MockBenefit[] {
	return mockBenefits.filter(
		(benefit) => benefit.userId === userId && benefit.isActive,
	);
}
