...
import { getBenefitsByUserId, getUserById } from "./mockData";
...
function calculateBikeLeasingSavings(
	benefit: Benefit,
	userSalary: number,
): { monthly: number; annual: number } {
	// Assume 42% tax bracket for high earners, 20% for lower
	const taxRate = userSalary > 4000 ? 0.42 : 0.2;
...
function calculatePensionSavings(benefit: Benefit): {
	monthly: number;
	annual: number;
} {
...
function calculateTaxSavingsForBenefit(
	benefit: Benefit,
	userSalary: number,
): { monthly: number; annual: number } {
...
export async function calculateTotalRewards(
	...
	try {
		...
		for (const benefit of userBenefits) {
			...
			const savings = calculateTaxSavingsForBenefit(
				benefitData,
				user.baseSalary,
			);
		...
		// Calculate totals
		const totalMonthlyComp =
			user.baseSalary + user.fixedBonus + benefitsValue + taxSavings;
		const ytdTotal = totalMonthlyComp * 12;

		// Generate transparency message
		const transparencyMessage = calculateTransparencyMessage(
			totalMonthlyComp,
			user.baseSalary,
		);
